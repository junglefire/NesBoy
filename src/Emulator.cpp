...
Emulator::Emulator(): m_cpu(m_bus), m_ppu(m_pictureBus, m_emulatorScreen), m_screenScale(3.f), m_cycleTimer(), m_cpuCycleDuration(NanoSeconds(559)) {
...
void Emulator::setVideoHeight(int height) {
    ...
    // LOG(Info) << "Scale: " << m_screenScale << " set. Screen: " << int(NESVideoWidth * m_screenScale) << "x" << int(NESVideoHeight * m_screenScale) << std::endl;
}
